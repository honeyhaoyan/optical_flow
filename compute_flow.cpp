...
std::string vid_path = "/Users/honey/Documents/Junior/vison/gpu_flow/avis/";

std::string out_path	= "/Users/honey/Documents/Junior/vison/gpu_flow/tvl1_flow/";
std::string out_path_jpeg	= "/Users/honey/Documents/Junior/vison/gpu_flow/rgb/";
...
bool captured(double time,double single){
	double option = time;
	if (time>1){
		time = time - (int)option;
	}
	if (time == 0) return true;
	if (time<=0.25 && time+single>=0.25) return true;
	if (time<=0.5 && time+single>=0.5) return true;
	if (time<=0.75 && time+single>=0.75) return true;
	if (time+single>=1) return true;
	return false;
}
...
int main( int argc, char *argv[] )
{
	...
	for (; (dirIt.hasNext()); )
	{
		...
		QString file = dirIt.fileName();
			...
			double total = 0;
			double single_frame_time = 1/(frameSkip*4);
			while( frame1.empty() == false )
			{
				...
				while(!captured(total,single_frame_time)){
					cap >> frame1_rgb_;
					total = total+single_frame_time;
				}
